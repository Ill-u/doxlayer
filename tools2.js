...
(function(w){
  ...
  AIO.biometricScrambler = function(el) {
    ...
    var fI=C.querySelector('#bioFile'),zone=C.querySelector('#bioZone'),
        ...
        dl=C.querySelector('#bioDl'),img=null,regions=[],cur=[];
    ...
    function load(file){var r=new FileReader();r.onload=function(e){img=new Image();img.onload=function(){cnv.width=img.naturalWidth;cnv.height=img.naturalHeight;redraw();prev.style.display='block';};img.src=e.target.result;};r.readAsDataURL(file);}
    function redraw(){var ctx=cnv.getContext('2d');ctx.drawImage(img,0,0);regions.forEach(function(reg){ctx.strokeStyle='rgba(239,68,68,0.7)';ctx.lineWidth=2;ctx.beginPath();reg.forEach(function(p,i){i===0?ctx.moveTo(p.x,p.y):ctx.lineTo(p.x,p.y);});ctx.closePath();ctx.stroke();ctx.fillStyle='rgba(239,68,68,0.15)';ctx.fill();});cur.forEach(function(p){ctx.beginPath();ctx.arc(p.x,p.y,5,0,Math.PI*2);ctx.fillStyle='#ef4444';ctx.fill();});}

    cnv.addEventListener('click',function(e){if(!img)return;var r=cnv.getBoundingClientRect();var x=(e.clientX-r.left)*(cnv.width/r.width),y=(e.clientY-r.top)*(cnv.height/r.height);cur.push({x:x,y:y});if(cur.length>=4){regions.push(cur.slice());cur=[];}redraw();});
    C.querySelector('#bioUndo').addEventListener('click',function(){if(cur.length)cur.pop();else if(regions.length)regions.pop();redraw();});
    ...
    C.querySelector('#bioApply').addEventListener('click',function(){
      ...
      var ctx=outC.getContext('2d');ctx.drawImage(img,0,0);
      var intensity=parseInt(rI.value),grid=parseInt(rG.value);
      regions.forEach(function(reg){
        var xs=reg.map(function(p){return p.x;}),ys=reg.map(function(p){return p.y;});
        var mnx=Math.floor(Math.min.apply(null,xs)),mxx=Math.ceil(Math.max.apply(null,xs));
        var mny=Math.floor(Math.min.apply(null,ys)),mxy=Math.ceil(Math.max.apply(null,ys));
        var w=mxx-mnx,h=mxy-mny;if(w<2||h<2)return;
        var id=ctx.getImageData(mnx,mny,w,h),d=id.data;
        for(var gy=0;gy<h;gy+=grid)for(var gx=0;gx<w;gx+=grid){
          var ox=Math.round((Math.random()-.5)*intensity*2),oy=Math.round((Math.random()-.5)*intensity*2);
          for(var py=gy;py<Math.min(gy+grid,h);py++)for(var px=gx;px<Math.min(gx+grid,w);px++){
            var sx=Math.max(0,Math.min(w-1,px+ox)),sy=Math.max(0,Math.min(h-1,py+oy));
            var di=(py*w+px)*4,si=(sy*w+sx)*4;d[di]=d[si];d[di+1]=d[si+1];d[di+2]=d[si+2];
          }
        }
        ctx.putImageData(id,mnx,mny);
      });
      dl.href=outC.toDataURL('image/png');res.classList.add('show');
    });
  };
  ...
  AIO.gdprLogicMesh=function(el){
    ...
    C.innerHTML='<div class="tool-sec"><label>Processing Purpose</label><input class="tool-inp" id="gdprPurpose" placeholder="e.g., Email Marketing Automation"/></div>'+
      ...
      ['Email','Name','Location','Behavior','Biometric','Financial'].map(function(c){return '<label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="'+c+'" style="accent-color:var(--accent)"/> '+c+'</label>';}).join('')+'</div></div>'+
    ...
    function addRule(){
      var div=document.createElement('div');div.style.cssText='display:grid;grid-template-columns:1fr auto 1fr auto;gap:8px;align-items:center;margin-bottom:8px;padding:12px;background:var(--bg-tertiary);border-radius:var(--radius-sm);border:1px solid var(--border)';
    ...
    C.querySelector('#gdprGen').addEventListener('click',function(){
      ...
      var cats=[];C.querySelectorAll('#gdprCats input:checked').forEach(function(cb){cats.push(cb.value);});
      var ruleEls=rulesDiv.children,logicRules=[];
      for(var i=0;i<ruleEls.length;i++){var sel=ruleEls[i].querySelectorAll('select');if(sel.length>=2)logicRules.push({condition:sel[0].value,action:sel[1].value});}
      var manifest={schema:'GDPR-LogicMesh-v2.0',generated:new Date().toISOString(),processing_purpose:purpose,data_categories:cats.length?cats:['Unspecified'],automated_decision_rules:logicRules,compliance:{article_22_automated_decisions:true,right_to_explanation:true,data_minimization:cats.length<=3},audit_trail:{version:'1.0',hash:'sha256:'+Array.from(crypto.getRandomValues(new Uint8Array(16))).map(function(b){return b.toString(16).padStart(2,'0');}).join('')}};
      var out=JSON.stringify(manifest,null,2);C.querySelector('#gdprOut').textContent=out;C.querySelector('#gdprRes').classList.add('show');
  ...
  AIO.shadowBanTest=function(el){
    ...
    C.querySelector('#sbBtn').addEventListener('click',function(){
      ...
      var ctx=canvas.getContext('2d');ctx.textBaseline='top';ctx.font='14px Arial';ctx.fillStyle='#f60';ctx.fillRect(125,1,62,20);ctx.fillStyle='#069';ctx.fillText('ShadowBan Test',2,15);ctx.fillStyle='rgba(102,204,0,0.7)';ctx.fillText('Canvas Hash',4,35);ctx.beginPath();ctx.arc(50,50,20,0,Math.PI*2);ctx.fillStyle='rgb(255,0,0)';ctx.fill();
      var gl=canvas.getContext('webgl');var debugInfo=gl?gl.getExtension('WEBGL_debug_renderer_info'):null;var gpu=debugInfo?gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL):'N/A';
      var dataUrl=canvas.toDataURL();var hash=0;for(var i=0;i<dataUrl.length;i++){hash=((hash<<5)-hash)+dataUrl.charCodeAt(i);hash|=0;}
      var hexHash=Math.abs(hash).toString(16).padStart(8,'0');
      var actx=new(window.AudioContext||window.webkitAudioContext)();var osc=actx.createOscillator();var analyser=actx.createAnalyser();var gain=actx.createGain();gain.gain.value=0;osc.connect(analyser);analyser.connect(gain);gain.connect(actx.destination);osc.start(0);var freq=new Float32Array(analyser.frequencyBinCount);analyser.getFloatFrequencyData(freq);var audioHash=0;for(var i=0;i<10;i++)audioHash+=Math.round(freq[i]*1000);osc.stop();actx.close();
      var screen=window.screen.width+'x'+window.screen.height+'@'+(window.devicePixelRatio||1);var tz=Intl.DateTimeFormat().resolvedOptions().timeZone;var langs=navigator.languages?navigator.languages.join(','):navigator.language;

      C.querySelector('#sbRes').innerHTML='<h3 style="margin-bottom:16px">Your Browser Fingerprint</h3><div style="display:grid;gap:8px">'+
        [['Canvas Hash',hexHash],['GPU Renderer',gpu||'Blocked'],['Audio Hash',Math.abs(audioHash).toString(16)],['Screen',screen],['Timezone',tz],['Languages',langs],['Platform',navigator.platform],['Touch Points',navigator.maxTouchPoints]].map(function(p){
  ...
  AIO.voiceJitterMask=function(el){
    ...
    var fI=C.querySelector('#vjFile'),zone=C.querySelector('#vjZone'),btn=C.querySelector('#vjApply'),res=C.querySelector('#vjRes'),audioEl=C.querySelector('#vjAudio'),dl=C.querySelector('#vjDl'),audioBuffer=null;
    zone.addEventListener('click',function(){fI.click();});
    fI.addEventListener('change',function(e){if(e.target.files.length){var r=new FileReader();r.onload=function(ev){var actx=new(window.AudioContext||window.webkitAudioContext)();actx.decodeAudioData(ev.target.result,function(buf){audioBuffer=buf;btn.style.display='inline-flex';actx.close();},function(){alert('Could not decode audio.');});};r.readAsArrayBuffer(e.target.files[0]);}});
    ...
    btn.addEventListener('click',function(){
      if(!audioBuffer)return;var actx=new(window.AudioContext||window.webkitAudioContext)();var sr=actx.sampleRate,len=audioBuffer.length,ch=audioBuffer.numberOfChannels;var outBuf=actx.createBuffer(ch,len,sr);
      var jitter=parseInt(C.querySelector('#vjInt').value)/100,harmonic=parseInt(C.querySelector('#vjHar').value);
      for(var c=0;c<ch;c++){var inp=audioBuffer.getChannelData(c),out=outBuf.getChannelData(c);for(var i=0;i<len;i++){out[i]=Math.max(-1,Math.min(1,inp[i]+(Math.random()-.5)*2*jitter*.1+Math.sin(i*harmonic*.001)*jitter*.05));}}
      var wav=encodeWAV(outBuf);var blob=new Blob([wav],{type:'audio/wav'});var url=URL.createObjectURL(blob);audioEl.src=url;dl.href=url;res.classList.add('show');actx.close();
    });
    function encodeWAV(buffer){var sr=buffer.sampleRate,ch=buffer.numberOfChannels,bps=16,samples=[];for(var c=0;c<ch;c++){var d=buffer.getChannelData(c);for(var i=0;i<d.length;i++){var s=Math.max(-1,Math.min(1,d[i]));samples.push(s<0?s*0x8000:s*0x7FFF);}}var ba=ch*bps/8,br=sr*ba,ds=samples.length*bps/8,buf=new ArrayBuffer(44+ds),v=new DataView(buf);
      function ws(o,s){for(var i=0;i<s.length;i++)v.setUint8(o+i,s.charCodeAt(i));}
      ws(0,'RIFF');v.setUint32(4,36+ds,true);ws(8,'WAVE');ws(12,'fmt ');v.setUint32(16,16,true);v.setUint16(20,1,true);v.setUint16(22,ch,true);v.setUint32(24,sr,true);v.setUint32(28,br,true);v.setUint16(32,ba,true);v.setUint16(34,bps,true);ws(36,'data');v.setUint32(40,ds,true);for(var i=0;i<samples.length;i++)v.setInt16(44+i*2,samples[i],true);return buf;}
  };

  // 20. Smart-Home Data Auditor
  AIO.smartHomeAudit=function(el){
    var C=document.getElementById(el);if(!C)return;
    var suspicious=[{p:/amazonaws\.com/i,r:'high',l:'AWS Cloud Upload'},{p:/google-analytics|googletagmanager/i,r:'medium',l:'Google Analytics'},{p:/facebook\.com\/tr|fbcdn/i,r:'high',l:'Facebook Tracking'},{p:/doubleclick\.net/i,r:'medium',l:'DoubleClick Ads'},{p:/telemetry|metrics|analytics/i,r:'low',l:'Generic Telemetry'},{p:/api\.amazon|alexa/i,r:'medium',l:'Amazon Alexa'},{p:/samsung|smartthings/i,r:'medium',l:'Samsung SmartThings'},{p:/tuya|smart-life/i,r:'medium',l:'Tuya IoT'},{p:/voice|speech|microphone/i,r:'high',l:'Voice Data'},{p:/location|gps|geoloc/i,r:'high',l:'Location Data'},{p:/biometric|face.*id/i,r:'high',l:'Biometric Data'},{p:/encrypt|ssl|tls/i,r:'info',l:'Encrypted (Good)'},{p:/firmware.*update|ota/i,r:'low',l:'Firmware Update'},{p:/1\.1\.1\.1|8\.8\.8\.8/i,r:'low',l:'DNS Query'}];
    ...
    C.querySelector('#iotBtn').addEventListener('click',function(){
      var text=C.querySelector('#iotIn').value;if(!text.trim())return;
      var lines=text.split('\n').filter(function(l){return l.trim().length>0;}),findings=[],rc={high:0,medium:0,low:0,info:0};
      lines.forEach(function(line,idx){suspicious.forEach(function(s){if(s.p.test(line)){findings.push({line:idx+1,text:line.trim().substring(0,120),risk:s.r,label:s.l});rc[s.r]++;}});});
      var total=rc.high*3+rc.medium*2+rc.low;var grade=total===0?'A+':total<=3?'A':total<=8?'B':total<=15?'C':'D';
      ...
      [{k:'high',l:'High',c:'badge-r'},{k:'medium',l:'Medium',c:'badge-y'},{k:'low',l:'Low',c:'badge-g'},{k:'info',l:'Info',c:'badge-g'}].forEach(function(x){h+='<span class="badge '+x.c+'">'+x.l+': '+rc[x.k]+'</span>';});h+='</div>';
      if(findings.length){h+='<div style="max-height:300px;overflow-y:auto">';findings.forEach(function(f){var col=f.risk==='high'?'var(--red)':f.risk==='medium'?'var(--amber)':'var(--accent)';h+='<div style="padding:10px 14px;margin-bottom:6px;background:var(--bg-tertiary);border-radius:var(--radius-sm);border-left:3px solid '+col+';font-size:.85rem"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="color:'+col+';font-weight:600">'+f.label+'</span><span style="color:var(--text-muted);font-size:.75rem">Line '+f.line+'</span></div><code style="font-family:var(--font-mono);font-size:.75rem;color:var(--text-muted);word-break:break-all">'+f.text+'</code></div>';});h+='</div>';}
      else{h+='<div style="text-align:center;padding:20px"><i class="fas fa-shield-halved" style="font-size:2rem;color:var(--green);margin-bottom:8px;display:block"></i><p style="color:var(--green);font-weight:600">No suspicious patterns detected</p></div>';}
  ...
  AIO.binaryBurner=function(el){
    ...
    var fI=C.querySelector('#binFile'),zone=C.querySelector('#binZone'),btn=C.querySelector('#binApply'),res=C.querySelector('#binRes'),dl=C.querySelector('#binDl'),fileData=null,fileName='';
    zone.addEventListener('click',function(){fI.click();});
    fI.addEventListener('change',function(e){if(!e.target.files.length)return;var file=e.target.files[0];fileName=file.name;var r=new FileReader();r.onload=function(ev){fileData=new Uint8Array(ev.target.result);btn.style.display='inline-flex';};r.readAsArrayBuffer(file);});

    function crc32(buf){var t=crc32.t||(crc32.t=(function(){var t=[];for(var n=0;n<256;n++){var c=n;for(var k=0;k<8;k++)c=(c&1)?(0xEDB88320^(c>>>1)):(c>>>1);t[n]=c;}return t;})());var c=0xFFFFFFFF;for(var i=0;i<buf.length;i++)c=(c>>>8)^t[(c^buf[i])&0xFF];return(c^0xFFFFFFFF)>>>0;}
    function makeChunk(keyword,text){var kw=new TextEncoder().encode(keyword),tx=new TextEncoder().encode(text),dl=kw.length+1+tx.length,chunk=new Uint8Array(12+dl),v=new DataView(chunk.buffer);v.setUint32(0,dl,false);chunk.set([0x74,0x45,0x58,0x74],4);chunk.set(kw,8);chunk[8+kw.length]=0;chunk.set(tx,8+kw.length+1);v.setUint32(8+dl,crc32(chunk.slice(4,8+dl)),false);return chunk;}

    btn.addEventListener('click',function(){
      if(!fileData)return;var level=C.querySelector('#binLevel').value;var bytes=new Uint8Array(fileData);var markers=[];
      if(fileName.endsWith('.png')){
        var t1=makeChunk('ai-training-opt-out','DO NOT USE FOR AI TRAINING');var t2=makeChunk('noai','true');
        var nb=new Uint8Array(bytes.length+t1.length+t2.length);var ihdrLen=(bytes[4]<<24)|(bytes[5]<<16)|(bytes[6]<<8)|bytes[7];var at=12+ihdrLen+4;
        nb.set(bytes.slice(0,at),0);nb.set(t1,at);nb.set(t2,at+t1.length);nb.set(bytes.slice(at),at+t1.length+t2.length);bytes=nb;
        markers.push('PNG tEXt: ai-training-opt-out');markers.push('PNG tEXt: noai=true');
        if(level==='hard'){var t3=makeChunk('poison','AI_SCRAPER_REJECT');var fb=new Uint8Array(bytes.length+t3.length);for(var i=bytes.length-4;i>0;i--){if(bytes[i]===0x49&&bytes[i+1]===0x45&&bytes[i+2]===0x4E&&bytes[i+3]===0x44){fb.set(bytes.slice(0,i-4),0);fb.set(t3,i-4);fb.set(bytes.slice(i-4),i-4+t3.length);bytes=fb;break;}}markers.push('PNG tEXt: poison marker');}
      }else{
        var header='/* [NO-AI-TRAINING] Protected content. */\n';var orig=new TextDecoder().decode(bytes);
        if(level!=='soft'){var p='';for(var i=0;i<orig.length;i++){p+=orig[i];if(i%3===0)p+='\u200B';if(i%7===0)p+='\u200D';}bytes=new TextEncoder().encode(header+p);markers.push('Zero-width injection');}else{bytes=new TextEncoder().encode(header+orig);}
        markers.push('Header: NO-AI-TRAINING');
      }
      var blob=new Blob([bytes],{type:'application/octet-stream'});var url=URL.createObjectURL(blob);dl.href=url;dl.download='protected-'+fileName;
  ...
  AIO.skillDecay=function(el){
    var C=document.getElementById(el);if(!C)return;
    var roles={'Frontend Developer':{hl:2.1,sk:['React','TypeScript','CSS','Web Components','WASM']},'Backend Developer':{hl:2.8,sk:['Node.js','Go','Rust','Kubernetes','gRPC']},'Data Scientist':{hl:1.5,sk:['Python','LLM Fine-tuning','MLOps','Statistics','Data Engineering']},'DevOps Engineer':{hl:1.8,sk:['Kubernetes','Terraform','GitOps','Platform Engineering','SRE']},'UX Designer':{hl:3.0,sk:['Figma','Design Systems','AI Design','Accessibility','Spatial Design']},'Product Manager':{hl:3.5,sk:['AI Product Strategy','Data-Driven','User Research','Growth','Ethics']},'Marketing Manager':{hl:1.9,sk:['AI Content','Performance Marketing','Attribution','Community','Brand Safety']},'AI/ML Engineer':{hl:0.9,sk:['LLM Architecture','RLHF','Prompt Engineering','Model Optimization','AI Safety']},'Content Writer':{hl:1.0,sk:['AI-Human Collab','SEO 2026','Storytelling','Fact-Checking','Voice Dev']},'Cybersecurity Analyst':{hl:1.2,sk:['AI Threat Detection','Zero Trust','Cloud Security','Incident Response','Compliance']},'Graphic Designer':{hl:2.0,sk:['AI Image Tools','Motion Design','3D Design','Brand Systems','AR/VR']},'HR Manager':{hl:2.5,sk:['AI Recruiting','DEI Analytics','Remote Culture','Skills Architecture','Ethical AI']},'Accountant':{hl:3.2,sk:['AI Bookkeeping','Crypto Accounting','ESG Reporting','Strategic Finance','RegTech']}};
    ...
    C.querySelector('#sdBtn').addEventListener('click',function(){
      var role=C.querySelector('#sdRole').value,dateStr=C.querySelector('#sdDate').value,data=roles[role];
      ...
      var last=new Date(dateStr),now=new Date(),ye=(now-last)/(365.25*864e5),hl=data.hl;
      var remaining=Math.pow(0.5,ye/hl)*100;var decay=new Date(last.getTime()+hl*365.25*864e5);var critical=new Date(last.getTime()+hl*2*365.25*864e5);
      var color=remaining>70?'var(--green)':remaining>40?'var(--amber)':'var(--red)';var status=remaining>70?'Current':remaining>40?'Decaying':'Critical';
      ...
      data.sk.forEach(function(s){h+='<span class="chip">'+s+'</span>';});h+='</div>';
  ...
  AIO.hitlRoi=function(el){
    ...
    C.querySelector('#hitlBtn').addEventListener('click',function(){
      var save=parseFloat(C.querySelector('#hitlSave').value)||0,cust=parseFloat(C.querySelector('#hitlCust').value)||0,err=parseFloat(C.querySelector('#hitlErr').value)||0,clv=parseFloat(C.querySelector('#hitlCLV').value)||0,resolve=parseFloat(C.querySelector('#hitlResolve').value)||0,agent=parseFloat(C.querySelector('#hitlAgent').value)||0;
      var errors=cust*(err/100),frustration=errors*clv*0.15,resCost=errors*resolve,totalCost=frustration+resCost,netROI=save-totalCost;
      var humanCost=cust*0.05*agent,hybridROI=(save*0.6)-(humanCost*0.4)-(totalCost*0.3);
  ...
  AIO.trustScore=function(el){
    ...
    C.querySelector('#tsBtn').addEventListener('click',async function(){
      var desc=C.querySelector('#tsDesc').value,hours=C.querySelector('#tsHours').value,files=C.querySelector('#tsFiles').value,client=C.querySelector('#tsClient').value,name=C.querySelector('#tsName').value;
      var log={schema:'Human-Work-Log-v1.0',freelancer:name,client:client,description:desc,hours_logged:parseFloat(hours),files_delivered:parseInt(files),timestamp:new Date().toISOString(),work_pattern:{avg_session:(parseFloat(hours)/Math.max(1,parseInt(files))).toFixed(1),est_keystrokes:Math.round(parseFloat(hours)*4500),human_probability:'HIGH'}};
      try{var enc=new TextEncoder();var kp=await crypto.subtle.generateKey({name:'ECDSA',namedCurve:'P-256'},true,['sign','verify']);var sig=await crypto.subtle.sign({name:'ECDSA',hash:'SHA-256'},kp.privateKey,enc.encode(JSON.stringify(log)));var sigH=Array.from(new Uint8Array(sig)).map(function(b){return b.toString(16).padStart(2,'0');}).join('');log.proof={algo:'ECDSA-P256-SHA256',signature:sigH.substring(0,64)+'...',note:'Cryptographically signed at generation time.'};}catch(e){log.proof={note:'Web Crypto unavailable — informational only.'};}
  ...
  AIO.aiLiability=function(el){
    ...
    C.querySelector('#aiBtn').addEventListener('click',function(){
      var temp=parseFloat(C.querySelector('#aiTemp').value),domain=C.querySelector('#aiDomain').value,review=parseFloat(C.querySelector('#aiReview').value)/100,dec=parseFloat(C.querySelector('#aiDecisions').value),claim=parseFloat(C.querySelector('#aiClaim').value);
      var dm={content:1,medical:8,financial:6,legal:7,hiring:5,safety:15}[domain]||1;
            var hallRate=Math.min(0.35,0.02+temp*0.08)*dm;
      var unreviewed=dec*(1-review),expectedErrors=unreviewed*hallRate,monthlyRisk=expectedErrors*claim;
      var annualRisk=monthlyRisk*12,insCost=annualRisk*0.04;
  ...
  AIO.remoteTaxSync=function(el){
    ...
    function addTrip(){
      var div=document.createElement('div');div.style.cssText='display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:8px;margin-bottom:8px;padding:12px;background:var(--bg-tertiary);border-radius:var(--radius-sm);border:1px solid var(--border)';
    ...
    C.querySelector('#taxBtn').addEventListener('click',function(){
      var home=C.querySelector('#taxHome').value;if(!home)return;
      var tripEls=tripsDiv.children,log=[],warnings=[],year={};
      for(var i=0;i<tripEls.length;i++){
        var cn=tripEls[i].querySelector('.trip-country').value,sd=tripEls[i].querySelector('.trip-start').value,ed=tripEls[i].querySelector('.trip-end').value;
        if(!cn||!sd||!ed)continue;
        var days=Math.ceil((new Date(ed)-new Date(sd))/864e5);if(days<1)continue;
      ...
      var homeDays=365-log.reduce(function(s,t){return s+t.days;},0);year[home]=(year[home]||0)+homeDays;
      ...
      Object.keys(year).sort(function(a,b){return year[b]-year[a];}).forEach(function(c){
        var d=year[c];var pct=(d/365*100).toFixed(1);var col=d>=183?'var(--red)':d>=90?'var(--amber)':'var(--green)';
  ...
  AIO.empathyMatrix=function(el){
    ...
    var qDiv=C.querySelector('#emQ'),current=0,score=0,maxScore=0;
    function renderQ(){
      ...
      s.a.forEach(function(a,i){h+='<button class="tool-btn sec" style="width:100%;text-align:left;justify-content:flex-start;margin-bottom:8px;padding:14px 18px" data-idx="'+i+'">'+a.t+'</button>';});h+='</div>';
      qDiv.innerHTML=h;
      qDiv.querySelectorAll('.tool-btn.sec').forEach(function(btn){
        btn.addEventListener('click',function(){
          var idx=parseInt(this.dataset.idx);score+=s.a[idx].s;maxScore+=3;current++;renderQ();
        });
      });
    }
    function showResults(){
      qDiv.innerHTML='';var pct=Math.round(score/maxScore*100);
  ...
  AIO.vendorLockin=function(el){
    ...
    C.querySelector('#vlBtn').addEventListener('click',function(){
      var name=C.querySelector('#vlPlat').value,rev=parseFloat(C.querySelector('#vlRev').value)||0,email=parseInt(C.querySelector('#vlEmail').value)||0;
      var p=platforms.find(function(x){return x.name===name;});if(!p)return;
      var lockin=Math.round((5-p.api)*8+(5-p.export)*12+p.fees*6+(5-p.audience)*4);
      var migrLoss=Math.round(rev*(lockin/100)*0.8);var recovery=Math.round(migrLoss/(rev*0.15));
  ...
  AIO.relatabilityScore=function(el){
    ...
    C.querySelector('#rsBtn').addEventListener('click',function(){
      var txt=C.querySelector('#rsIn').value;if(!txt.trim())return;
      var words=txt.split(/\s+/).filter(function(w){return w.length>0;}),wc=words.length;
      ...
      var varSent=0;var lens=sents.map(function(s){return s.split(/\s+/).length;});var avg=lens.reduce(function(a,b){return a+b;},0)/Math.max(1,lens.length);lens.forEach(function(l){varSent+=(l-avg)*(l-avg);});varSent=Math.sqrt(varSent/Math.max(1,lens.length));
      ...
      if(tips.length){
        ...
        tips.forEach(function(t){h+='<p style="font-size:.85rem;color:var(--text-secondary);margin-bottom:4px">• '+t+'</p>';});h+='</div>';
      }
  ...
  AIO.sourceTimestamp=function(el){
    ...
    C.querySelector('#stBtn').addEventListener('click',async function(){
      var title=C.querySelector('#stTitle').value,body=C.querySelector('#stBody').value,author=C.querySelector('#stAuthor').value;
      ...
      var enc=new TextEncoder();var hashBuf=await crypto.subtle.digest('SHA-256',enc.encode(title+body));
      var hash=Array.from(new Uint8Array(hashBuf)).map(function(b){return b.toString(16).padStart(2,'0');}).join('');
      var now=new Date();var ntpSync=false;try{var r=await fetch('https://worldtimeapi.org/api/timezone/Etc/UTC',{signal:AbortSignal.timeout(3000)});if(r.ok){var d=await r.json();ntpSync=true;now=new Date(d.datetime);}}catch(e){}

      var cert={schema:'First-Published-Certificate-v1.0',title:title,author:author,content_hash:'sha256:'+hash.substring(0,32),timestamp_utc:now.toISOString(),timestamp_epoch:now.getTime(),ntp_verified:ntpSync,method:ntpSync?'NTP-synced (worldtimeapi.org)':'Local clock (connect to internet for NTP verification)',instructions:'To verify: re-hash your original content with SHA-256. If the hash matches, the content existed at the stated timestamp.'};
      var json=JSON.stringify(cert,null,2);

      // Store in localStorage for future proof
      var key='sot-'+hash.substring(0,16);var existing=localStorage.getItem(key);
      if(existing){var prev=JSON.parse(existing);cert.note='Earlier certificate found: '+prev.timestamp_utc+'. This content predates the current timestamp.';}
  ...
  AIO.burnoutSentiment=function(el){
    ...
    C.querySelector('#bsBtn').addEventListener('click',function(){
      var niche=C.querySelector('#bsNiche').value,output=parseInt(C.querySelector('#bsOutput').value)||12;
      var data=niches[niche];if(!data)return;
      var health=100-data.saturation+data.growth;var color=health>50?'var(--green)':health>20?'var(--amber)':'var(--red)';
      ...
      data.pivot.forEach(function(p){h+='<span class="chip"><i class="fas fa-arrow-right" style="font-size:.7rem"></i> '+p+'</span>';});h+='</div>';
  ...
  AIO.trustTax=function(el){
    ...
    C.querySelector('#ttBtn').addEventListener('click',function(){
      var followers=parseFloat(C.querySelector('#ttFollow').value)||0,engage=parseFloat(C.querySelector('#ttEngage').value)/100,rev=parseFloat(C.querySelector('#ttRev').value)||0,dms=parseFloat(C.querySelector('#ttDms').value)||0,repeat=parseFloat(C.querySelector('#ttRepeat').value)/100,real=parseFloat(C.querySelector('#ttReal').value)/100;
      var engaged=followers*engage,botEst=engaged*(1-real),humanEngaged=engaged*real;
      var revenuePerFollower=rev/followers,trustAdjusted=rev/(followers*real);
      var trustTax=rev*(1-real*repeat)*0.3;var lostRevenue=followers*0.001-rev/followers*1000;
      ...
      h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:20px">';
      [['Human Engaged',Math.round(humanEngaged).toLocaleString()],['Bot/Passive',Math.round(botEst).toLocaleString()],['Trust Tax/mo','$'+Math.round(trustTax)+'/mo'],['Rev/Follower','$'+revenuePerFollower.toFixed(3)],['Trust-Adjusted Rev','$'+trustAdjusted.toFixed(2)+'/follower'],['Repeat Rate',(repeat*100).toFixed(0)+'%']].forEach(function(d){
        h+='<div style="text-align:center;padding:14px;background:var(--bg-tertiary);border-radius:var(--radius-sm)"><p style="font-size:.75rem;color:var(--text-muted)">'+d[0]+'</p><p style="font-weight:700">'+d[1]+'</p></div>';});
  ...
  AIO.voiceHarmonizer=function(el){
    ...
    C.querySelector('#vhBtn').addEventListener('click',function(){
      var txt=C.querySelector('#vhIn').value,from=C.querySelector('#vhFrom').value,to=C.querySelector('#vhTo').value;
      var traits=[];C.querySelectorAll('#vhTraits input:checked').forEach(function(cb){traits.push(cb.value);});
      if(!txt.trim())return;
      ...
      r.add.forEach(function(t){h+='<p style="font-size:.85rem;color:var(--text-secondary);margin-bottom:6px">• '+t+'</p>';});h+='</div>';
      ...
      r.remove.forEach(function(t){h+='<p style="font-size:.85rem;color:var(--text-secondary);margin-bottom:6px">• '+t+'</p>';});h+='</div></div>';
      ...
      if(traits.length){
        ...
        traits.forEach(function(t){
          var tips={Direct:'Keep your directness. Start sentences with "Here\'s the deal:" or "Bottom line:"',Casual:'Use contractions and conversational connectors. "Look," "Here\'s the thing," "Honestly?"',Data:'Lead with a number. "73% of…" or "In the last 30 days…"','Storyteller':'Open with a micro-story. "Last Tuesday, I…" or "A client told me…"',Opinionated:'State your position early. "I believe…" or "Unpopular opinion:"',Technical:'Include specific tool names, version numbers, or code snippets where relevant.',Humorous:'Add a self-deprecating aside or a witty observation. Don\'t force it.',Empathetic:'Acknowledge the reader\'s struggle. "I know this is frustrating because…" or "If you\'ve ever felt…"'};
  ...
  AIO.spatialMetadata=function(el){
    ...
    C.querySelector('#spBtn').addEventListener('click',function(){
      var name=C.querySelector('#spName').value,desc=C.querySelector('#spDesc').value,w=C.querySelector('#spW').value,h=C.querySelector('#spH').value,d=C.querySelector('#spD').value,url=C.querySelector('#spUrl').value,cat=C.querySelector('#spCat').value;
      var meta='<meta name="ar-spatial-entity" content="'+name+'"/>\n'+
        ...
        JSON.stringify({'@context':'https://schema.org','@type':'3DModel',name:name,description:desc,'@id':'#spatial-'+name.toLowerCase().replace(/\s+/g,'-'),encodingFormat:'model/vnd.usdz+zip',contentUrl:url,spatialCoverage:{'@type':'Place',geo:{'@type':'GeoCoordinates',description:'Indoor object placement'}},additionalProperty:[{'@type':'PropertyValue',name:'width',value:w+'m'},{'@type':'PropertyValue',name:'height',value:h+'m'},{'@type':'PropertyValue',name:'depth',value:d+'m'}]},null,2)+'\n<\/script>\n'+
  ...
  AIO.visualContrastSchema=function(el){
    ...
    var fI=C.querySelector('#vcFile'),zone=C.querySelector('#vcZone'),cnv=C.querySelector('#vcCnv'),img=null;
    zone.addEventListener('click',function(){fI.click();});
    fI.addEventListener('change',function(e){if(e.target.files.length){var r=new FileReader();r.onload=function(ev){img=new Image();img.onload=function(){cnv.width=img.naturalWidth;cnv.height=img.naturalHeight;cnv.getContext('2d').drawImage(img,0,0);analyze();};img.src=ev.target.result;};r.readAsDataURL(e.target.files[0]);}});

    function analyze(){
      var ctx=cnv.getContext('2d'),w=cnv.width,h=cnv.height,d=ctx.getImageData(0,0,w,h).data;
      var bins={};for(var i=0;i<d.length;i+=16){var r=d[i],g=d[i+1],b=d[i+2];var hsl=rgbToHsl(r,g,b);var bin=Math.round(hsl[0]/30)*30+','+Math.round(hsl[1]/25)*25+','+Math.round(hsl[2]/25)*25;bins[bin]=(bins[bin]||0)+1;}
      ...
      sorted.forEach(function(s){var c=s[0].split(',').map(Number);var hex=hslToHex(c[0],c[1],c[2]);h+='<div style="width:60px;height:60px;border-radius:var(--radius-sm);background:'+hex+';display:flex;align-items:end;justify-content:center;padding:4px"><span style="font-size:.6rem;color:#fff;background:rgba(0,0,0,.5);padding:1px 4px;border-radius:4px">'+hex+'</span></div>';});
    ...
    function rgbToHsl(r,g,b){r/=255;g/=255;b/=255;var mx=Math.max(r,g,b),mn=Math.min(r,g,b),h,s,l=(mx+mn)/2;if(mx===mn){h=s=0;}else{var d=mx-mn;s=l>0.5?d/(2-mx-mn):d/(mx+mn);switch(mx){case r:h=((g-b)/d+(g<b?6:0))/6;break;case g:h=((b-r)/d+2)/6;break;case b:h=((r-g)/d+4)/6;break;}}return[Math.round(h*360),Math.round(s*100),Math.round(l*100)];}
    function hslToHex(h,s,l){s/=100;l/=100;var a=s*Math.min(l,1-l);var f=function(n){var k=(n+h/30)%12;return l-a*Math.max(Math.min(k-3,9-k,1),-1);};var r=Math.round(f(0)*255),g=Math.round(f(8)*255),b=Math.round(f(4)*255);return '#'+[r,g,b].map(function(x){return x.toString(16).padStart(2,'0');}).join('');}
  };
  ...
  AIO.cognitiveLoad=function(el){
    ...
    C.querySelector('#clBtn').addEventListener('click',function(){
      var txt=C.querySelector('#clIn').value;if(!txt.trim())return;
      ...
      var avgSentLen=wc/Math.max(1,sents.length);
      var uniqueWords={};words.forEach(function(w){uniqueWords[w.toLowerCase()]=true;});
      var vocabRichness=Object.keys(uniqueWords).length/Math.max(1,wc);

      // Flesch approximation
      var syllables=0;words.forEach(function(w){w=w.toLowerCase().replace(/[^a-z]/g,'');if(!w)return;var s=0,prev=false;for(var i=0;i<w.length;i++){var v='aeiouy'.indexOf(w[i])!==-1;if(v&&!prev)s++;prev=v;}syllables+=Math.max(1,s);});
      ...
      [['Cognitive Load',loadScore+'/100'],['Time to Digest',ttd+' min'],['Scannability',scannability+'/100'],['Words',wc],['Avg Sentence',Math.round(avgSentLen)+' words'],['Flesch',Math.round(flesch)],['Headings',headings],['Images',images],['Paragraphs',paragraphs.length],['Unique Words',Object.keys(uniqueWords).length]].forEach(function(d){
        h+='<div style="text-align:center;padding:14px;background:var(--bg-tertiary);border-radius:var(--radius-sm)"><p style="font-size:.75rem;color:var(--text-muted)">'+d[0]+'</p><p style="font-weight:700">'+d[1]+'</p></div>';});
      h+='</div>';
  ...
  AIO.zeroClickRevenue=function(el){
    ...
    C.querySelector('#zcBtn').addEventListener('click',function(){
      ...
      var cpc=parseFloat(C.querySelector('#zcCpc').value)||1.5,ctr=parseFloat(C.querySelector('#zcCtr').value)/100||0.28;
      var keywords=[],totalTraffic=0,totalLost=0,totalRevLost=0;
      raw.forEach(function(line){
        var parts=line.trim().split(/\s+/);var vol=parseInt(parts.pop());if(isNaN(vol))return;
        var kw=parts.join(' ');if(!kw)return;
        // Estimate zero-click probability based on keyword type
        var zcProb=0.65; // default
        ...
        var traffic=Math.round(vol*ctr),lost=Math.round(traffic*zcProb),revLost=lost*cpc;
  ...
  AIO.darkTraffic=function(el){
    ...
    C.querySelector('#dtBtn').addEventListener('click',function(){
      var url=C.querySelector('#dtUrl').value.replace(/\/$/,''),path=C.querySelector('#dtPath').value,camp=C.querySelector('#dtCamp').value;
  ...
  AIO.eeatGraph=function(el){
    ...
    C.querySelector('#eatBtn').addEventListener('click',function(){
      var name=C.querySelector('#eatName').value,topic=C.querySelector('#eatTopic').value,
  ...
  AIO.microIntentTree=function(el){
    ...
    C.querySelector('#miBtn').addEventListener('click',function(){
      var seed=C.querySelector('#miSeed').value.trim().toLowerCase();if(!seed)return;
      ...
      for(var d=1;d<=depth;d++){
        ...
        mods.forEach(function(mod){
          var intent=mod+' '+seed;
          intents.push({level:d,text:intent,competition:d===1?'high':d===2?'medium':'low'});
        });
      }
      ...
      var byLevel={};intents.forEach(function(i){if(!byLevel[i.level])byLevel[i.level]=[];byLevel[i.level].push(i);});
      ...
      C.querySelector('#miCp').addEventListener('click',function(){
        ...
        var b=this;setTimeout(function(){b.innerHTML='<i class="fas fa-copy"></i> Copy All';},2000);
      });
    });
  };
  ...
  AIO.byteToCarbon=function(el){
    ...
    C.querySelector('#bcBtn').addEventListener('click',function(){
      var html=C.querySelector('#bcIn').value,views=parseInt(C.querySelector('#bcViews').value)||50000;
      if(!html.trim())return;
      ...
      // Estimates per page load
      var estJsBytes=scripts*45000;var estImgBytes=images*150000;var estCssBytes=cssFiles*20000;var estFontBytes=fonts*80000;
      var totalBytes=htmlBytes+estJsBytes+estImgBytes+estCssBytes+estFontBytes;
      var totalKB=Math.round(totalBytes/1024);var totalMB=(totalBytes/1048576).toFixed(2);

      // Energy: ~0.81 kWh per GB transferred (IEA 2024 estimate)
      var kwhPerGB=0.81;var gramsPerKwh=442; // global average
      var gbPerView=totalBytes/1073741824;
      var kwhPerMonth=gbPerView*views*kwhPerGB;
      var co2PerMonth=kwhPerMonth*gramsPerKwh; // grams
      var co2PerYear=co2PerMonth*12;

      // Comparison
      var kmDriven=(co2PerYear/1000)*4.6; // ~230g CO2/km average car
      var treesNeeded=Math.ceil(co2PerYear/22000); // ~22kg CO2/tree/year
      ...
      [['Page Size',totalKB+' KB'],['Monthly CO₂',Math.round(co2PerMonth)+'g'],['Yearly CO₂',(co2PerYear/1000).toFixed(1)+'kg'],['Equiv. Driving',kmDriven.toFixed(0)+' km/yr'],['Trees Needed',treesNeeded],['Requests',scripts+images+cssFiles+fonts],['3rd Party',thirdParty],['Monthly kWh',kwhPerMonth.toFixed(3)]].forEach(function(d){
        h+='<div style="text-align:center;padding:14px;background:var(--bg-tertiary);border-radius:var(--radius-sm)"><p style="font-size:.75rem;color:var(--text-muted)">'+d[0]+'</p><p style="font-weight:700">'+d[1]+'</p></div>';});
      h+='</div>';
  ...
  AIO.ethicalBiasSim=function(el){
    ...
    C.querySelector('#ebBtn').addEventListener('click',function(){
      var desc=C.querySelector('#ebDesc').value;if(!desc.trim()){return;}
      var attrs=[];C.querySelectorAll('#ebAttrs input:checked').forEach(function(cb){attrs.push(cb.value);});
      if(!attrs.length)return;

      var scenarios=[];var totalTests=0;var issues=0;
      ...
      var byAttr={};scenarios.forEach(function(s){if(!byAttr[s.attr])byAttr[s.attr]=[];byAttr[s.attr].push(s);});
  ...
  AIO.burstinessStylometer=function(el){
    ...
    C.querySelector('#bsBtn').addEventListener('click',function(){
      var txt=C.querySelector('#bsIn').value;if(!txt.trim())return;
      ...
      var n=lens.length;if(n<3){C.querySelector('#bsRes').innerHTML='<p style="color:var(--red)">Need at least 3 sentences.</p>';C.querySelector('#bsRes').classList.add('show');return;}

      var mean=lens.reduce(function(a,b){return a+b;},0)/n;
      var variance=lens.reduce(function(a,b){return a+(b-mean)*(b-mean);},0)/n;
      var std=Math.sqrt(variance);
      var cv=std/mean; // coefficient of variation — key burstiness metric
      var burstiness=cv;

      // Perplexity proxy: word frequency distribution
      ...
      var total=words.length;var entropy=0;
      Object.values(freq).forEach(function(c){var p=c/total;entropy-=p*Math.log2(p);});
      var perplexity=Math.pow(2,entropy);

      // Repetition detection
      var bigrams={};for(var i=0;i<words.length-1;i++){var bg=words[i]+' '+words[i+1];bigrams[bg]=(bigrams[bg]||0)+1;}
      ...
      if(mean>18&&std<5)aiScore+=20; // uniform sentence length
      if(perplexity<50)aiScore+=15; // low vocabulary diversity
      ...
      h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;margin-bottom:20px">';
      [['Burstiness',burstiness.toFixed(3)],['Sent. Std Dev',std.toFixed(1)],['Avg Length',mean.toFixed(1)+'w'],['Entropy',entropy.toFixed(2)],['Perplexity',perplexity.toFixed(0)],['Repetition',(repetitionRate*100).toFixed(1)+'%'],['Transitions',transitions],['Sentences',n]].forEach(function(d){
      ...
      lens.forEach(function(l){
        var pct=(l/maxLen*100);var bc=pct>80?'var(--accent)':pct>50?'var(--amber)':'var(--text-muted)';
  ...
  AIO.sgeGapFinder=function(el){
    ...
    C.querySelector('#sgBtn').addEventListener('click',function(){
      var topic=C.querySelector('#sgTopic').value.trim();if(!topic)return;
      ...
      gapTypes.forEach(function(gt){
        ...
        gt.gaps.forEach(function(g){
          // Replace generic placeholders with topic

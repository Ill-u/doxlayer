...
(function(w){
  ...
  /* Helper: safely create a tool container */
  ...
  AIO.biometricScrambler = function(el) {
    var C = initTool(el, '<div class="tool-sec"><label>Upload Photo</label><div class="file-zone" id="bioZone"><i class="fas fa-face-smile"></i><p>Upload a photo to scramble facial landmarks</p><p class="sm">JPG, PNG — processed locally</p><input type="file" id="bioFile" accept="image/*"/></div></div><div class="tool-sec"><label>Scramble Intensity: <span id="bioIv">3</span>px</label><input type="range" id="bioInt" min="1" max="10" value="3" style="width:100%;accent-color:var(--accent)"/></div><div class="tool-sec"><label>Grid Size: <span id="bioGv">16</span>px</label><input type="range" id="bioGrid" min="4" max="32" value="16" style="width:100%;accent-color:var(--accent)"/></div><p style="color:var(--text-muted);font-size:.85rem;margin-bottom:12px">Click 4 corners on the image to mark each face region. Add multiple regions.</p><div class="tool-sec" id="bioPrev" style="display:none"><label>Image — Click to Mark Face Regions</label><canvas id="bioCnv" style="max-width:100%;border-radius:var(--radius-sm);border:1px solid var(--border);cursor:crosshair"></canvas><div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap"><button class="tool-btn sec" id="bioUndo"><i class="fas fa-undo"></i> Undo</button><button class="tool-btn" id="bioApply"><i class="fas fa-shield-halved"></i> Scramble</button></div></div><div id="bioRes" class="tool-res"><p style="color:var(--green);font-weight:600;margin-bottom:12px"><i class="fas fa-check-circle"></i> Landmarks scrambled!</p><canvas id="bioOut" style="max-width:100%;border-radius:var(--radius-sm);margin-bottom:12px"></canvas><a class="tool-btn" id="bioDl" download="scrambled.png"><i class="fas fa-download"></i> Download</a></div>');
    if (!C) return;
    ...
    function load(file) {
      ...
      r.onload = function(e) {
        img = new Image();
        img.onload = function() { cnv.width = img.naturalWidth; cnv.height = img.naturalHeight; redraw(); prev.style.display = 'block'; };
    ...
    function redraw() {
      ...
      cur.forEach(function(p) { ctx.beginPath(); ctx.arc(p.x, p.y, 5, 0, Math.PI * 2); ctx.fillStyle = '#ef4444'; ctx.fill(); });
    }

    cnv.addEventListener('click', function(e) {
      if (!img) return;
      var rect = cnv.getBoundingClientRect();
      var x = (e.clientX - rect.left) * (cnv.width / rect.width), y = (e.clientY - rect.top) * (cnv.height / rect.height);
      cur.push({ x: x, y: y });
    ...
    C.querySelector('#bioApply').addEventListener('click', function() {
      ...
      regions.forEach(function(reg) {
        ...
        var mnx = Math.floor(Math.min.apply(null, xs)), mxx = Math.ceil(Math.max.apply(null, xs)),
            mny = Math.floor(Math.min.apply(null, ys)), mxy = Math.ceil(Math.max.apply(null, ys));
        ...
        for (var gy = 0; gy < h; gy += grid) {
          for (var gx = 0; gx < w; gx += grid) {
            var ox = Math.round((Math.random() - 0.5) * intensity * 2), oy = Math.round((Math.random() - 0.5) * intensity * 2);
  ...
  AIO.gdprLogicMesh = function(el) {
    var C = initTool(el, '<div class="tool-sec"><label>Processing Purpose</label><input class="tool-inp" id="gdprPurpose" placeholder="e.g., Email Marketing Automation"/></div><div class="tool-sec"><label>Data Categories</label><div style="display:flex;gap:8px;flex-wrap:wrap" id="gdprCats"><label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="Email" style="accent-color:var(--accent)"/> Email</label><label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="Name" style="accent-color:var(--accent)"/> Name</label><label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="Location" style="accent-color:var(--accent)"/> Location</label><label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="Behavior" style="accent-color:var(--accent)"/> Behavior</label><label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="Biometric" style="accent-color:var(--accent)"/> Biometric</label><label style="display:flex;align-items:center;gap:6px;padding:8px 14px;background:var(--bg-tertiary);border-radius:var(--radius-full);cursor:pointer;font-size:.85rem;border:1px solid var(--border)"><input type="checkbox" value="Financial" style="accent-color:var(--accent)"/> Financial</label></div></div><div class="tool-sec"><label>Logic Rules</label><div id="gdprRules"></div><button class="tool-btn sec" id="gdprAdd" style="margin-top:8px"><i class="fas fa-plus"></i> Add Rule</button></div><button class="tool-btn" id="gdprGen"><i class="fas fa-file-code"></i> Generate JSON Manifest</button><div id="gdprRes" class="tool-res"><div class="tool-sec"><label>GDPR 2.0 Logic-Mesh Manifest</label><pre id="gdprOut" class="tool-inp" style="min-height:200px;white-space:pre-wrap;font-family:var(--font-mono);font-size:.8rem;cursor:pointer" title="Click to copy"></pre></div></div>');
    if (!C) return;
    ...
    C.querySelector('#gdprGen').addEventListener('click', function() {
      ...
      var logicRules = []; rulesDiv.querySelectorAll('select').forEach(function(sel, idx, list) {
        if (idx % 2 === 0 && list[idx + 1]) logicRules.push({ condition: sel.value, action: list[idx + 1].value });
      });
      var manifest = {
        ...
        audit_trail: { version: '1.0', hash: 'sha256:' + Array.from(crypto.getRandomValues(new Uint8Array(16))).map(function(b) { return b.toString(16).padStart(2, '0'); }).join('') }
      };
  ...
  AIO.shadowBanTest = function(el) {
    var C = initTool(el, '<div class="tool-sec"><p style="color:var(--text-secondary);margin-bottom:16px">Generates a multi-layer browser fingerprint from Canvas, WebGL, and AudioContext. Compare across devices/browsers to detect if platforms assign different trust levels to your sessions.</p></div><button class="tool-btn" id="sbBtn"><i class="fas fa-fingerprint"></i> Generate Fingerprint</button><div id="sbRes" class="tool-res"></div>');
    if (!C) return;
    C.querySelector('#sbBtn').addEventListener('click', function() {
      ...
      var ctx = canvas.getContext('2d'); ctx.textBaseline = 'top'; ctx.font = '14px Arial';
      ...
      var gl = canvas.getContext('webgl'), debugInfo = gl ? gl.getExtension('WEBGL_debug_renderer_info') : null,
          gpu = debugInfo ? gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL) : 'N/A';
      var dataUrl = canvas.toDataURL(); var hash = 0;
      for (var i = 0; i < dataUrl.length; i++) { hash = ((hash << 5) - hash) + dataUrl.charCodeAt(i); hash |= 0; }
      var hexHash = Math.abs(hash).toString(16).padStart(8, '0');
      var actx = new (window.AudioContext || window.webkitAudioContext)();
      var osc = actx.createOscillator(), analyser = actx.createAnalyser(), gain = actx.createGain();
      gain.gain.value = 0; osc.connect(analyser); analyser.connect(gain); gain.connect(actx.destination);
      osc.start(0); var freq = new Float32Array(analyser.frequencyBinCount); analyser.getFloatFrequencyData(freq);
      var audioHash = 0; for (var i = 0; i < 10; i++) audioHash += Math.round(freq[i] * 1000);
      osc.stop(); actx.close();
      ...
      var h = '<h3 style="margin-bottom:16px">Your Browser Fingerprint</h3><div style="display:grid;gap:8px">';
      [['Canvas Hash',hexHash],['GPU Renderer',gpu||'Blocked'],['Audio Hash',Math.abs(audioHash).toString(16)],['Screen',screen],['Timezone',tz],['Languages',langs],['Platform',navigator.platform],['Touch Points',navigator.maxTouchPoints]].forEach(function(p) {
        h += '<div style="display:flex;justify-content:space-between;padding:10px 14px;background:var(--bg-tertiary);border-radius:var(--radius-sm)"><span style="color:var(--text-muted);font-size:.85rem">' + p[0] + '</span><span style="font-family:var(--font-mono);font-size:.85rem;font-weight:600;color:var(--accent)">' + p[1] + '</span></div>';
      });
      h += '</div><div style="margin-top:16px;padding:16px;background:var(--bg-tertiary);border-radius:var(--radius-sm);border-left:3px solid var(--accent)"><p style="font-size:.85rem;color:var(--text-secondary)"><strong>Combined:</strong><br/><span style="font-family:var(--font-mono);font-size:.8rem;word-break:break-all;color:var(--accent)">' + hexHash + '-' + Math.abs(audioHash).toString(16) + '-' + screen.replace(/[@x]/g,'') + '</span></p></div><p style="margin-top:12px;color:var(--text-muted);font-size:.8rem">Open in a different browser or incognito mode. If the hash changes significantly, the platform may assign different trust scores to your sessions.</p>';
      C.querySelector('#sbRes').innerHTML = h; C.querySelector('#sbRes').classList.add('show');
    });
  };

  /* 19. Voice-Print Jitter Mask */
  AIO.voiceJitterMask = function(el) {
    var C = initTool(el, '<div class="tool-sec"><label>Upload Audio</label><div class="file-zone" id="vjZone"><i class="fas fa-microphone"></i><p>Upload a voice recording to protect</p><p class="sm">MP3, WAV, OGG — processed locally via Web Audio API</p><input type="file" id="vjFile" accept="audio/*"/></div></div><div class="tool-sec"><label>Jitter Intensity: <span id="vjIv">15</span>%</label><input type="range" id="vjInt" min="5" max="40" value="15" style="width:100%;accent-color:var(--accent)"/></div><div class="tool-sec"><label>Harmonic Shift: <span id="vjHv">3</span></label><input type="range" id="vjHar" min="1" max="8" value="3" style="width:100%;accent-color:var(--accent)"/></div><button class="tool-btn" id="vjApply" style="display:none"><i class="fas fa-shield-halved"></i> Apply Jitter Mask</button><div id="vjRes" class="tool-res"><p style="color:var(--green);font-weight:600;margin-bottom:12px"><i class="fas fa-check-circle"></i> Voice-print protected!</p><audio id="vjAudio" controls style="width:100%;margin-bottom:12px"></audio><a class="tool-btn" id="vjDl" download="protected-voice.wav"><i class="fas fa-download"></i> Download</a></div>');
    if (!C) return;
    var fI = C.querySelector('#vjFile'), zone = C.querySelector('#vjZone'), btn = C.querySelector('#vjApply'),
        res = C.querySelector('#vjRes'), audioEl = C.querySelector('#vjAudio'), dl = C.querySelector('#vjDl'), audioBuffer = null;
    zone.addEventListener('click', function() { fI.click(); });
    fI.addEventListener('change', function(e) {
      ...
      r.onload = function(ev) {
        var actx = new (window.AudioContext || window.webkitAudioContext)();
        actx.decodeAudioData(ev.target.result, function(buf) { audioBuffer = buf; btn.style.display = 'inline-flex'; actx.close(); }, function() { alert('Could not decode audio.'); });
      };
    ...
    btn.addEventListener('click', function() {
      ...
      var actx = new (window.AudioContext || window.webkitAudioContext)(), sr = actx.sampleRate;
      var len = audioBuffer.length, ch = audioBuffer.numberOfChannels, outBuf = actx.createBuffer(ch, len, sr);
      var jitter = parseInt(C.querySelector('#vjInt').value) / 100, harmonic = parseInt(C.querySelector('#vjHar').value);
      for (var c = 0; c < ch; c++) {
        var inp = audioBuffer.getChannelData(c), out = outBuf.getChannelData(c);
        for (var i = 0; i < len; i++) {
          out[i] = Math.max(-1, Math.min(1, inp[i] + (Math.random() - 0.5) * 2 * jitter * 0.1 + Math.sin(i * harmonic * 0.001) * jitter * 0.05));
        }
      }
      var wav = encodeWAV(outBuf), blob = new Blob([wav], { type: 'audio/wav' }), url = URL.createObjectURL(blob);
      audioEl.src = url; dl.href = url; res.classList.add('show'); actx.close();
    });
    function encodeWAV(buffer) {
      var sr = buffer.sampleRate, ch = buffer.numberOfChannels, bps = 16, samples = [];
      for (var c = 0; c < ch; c++) {
        var d = buffer.getChannelData(c);
        for (var i = 0; i < d.length; i++) { var s = Math.max(-1, Math.min(1, d[i])); samples.push(s < 0 ? s * 0x8000 : s * 0x7FFF); }
      }
      var ba = ch * bps / 8, br = sr * ba, ds = samples.length * bps / 8, buf = new ArrayBuffer(44 + ds), v = new DataView(buf);
      ...
      v.setUint32(40, ds, true); for (var i = 0; i < samples.length; i++) v.setInt16(44 + i * 2, samples[i], true);
      return buf;
    }
  };
  ...
  AIO.smartHomeAudit = function(el) {
    ...
    C.querySelector('#iotBtn').addEventListener('click', function() {
      ...
      lines.forEach(function(line, idx) {
        suspicious.forEach(function(s) { if (s.p.test(line)) { findings.push({ line: idx + 1, text: line.trim().substring(0, 120), risk: s.r, label: s.l }); rc[s.r]++; } });
      });
      var total = rc.high * 3 + rc.medium * 2 + rc.low, grade = total === 0 ? 'A+' : total <= 3 ? 'A' : total <= 8 ? 'B' : total <= 15 ? 'C' : 'D';
  ...
  AIO.binaryBurner = function(el) {
    var C = initTool(el, '<div class="tool-sec"><label>Upload File to Protect</label><div class="file-zone" id="binZone"><i class="fas fa-file-shield"></i><p>Upload PNG or TXT file</p><p class="sm">Injects anti-AI training markers into file binary</p><input type="file" id="binFile" accept=".png,.txt,.jpg,.jpeg"/></div></div><div class="tool-sec"><label>Protection Level</label><select class="tool-inp" id="binLevel"><option value="soft">Soft — Standard opt-out tags</option><option value="medium" selected>Medium — Tag + Header Poison</option><option value="hard">Hard — Maximum disruption</option></select></div><button class="tool-btn" id="binApply" style="display:none"><i class="fas fa-fire"></i> Burn Anti-AI Markers</button><div id="binRes" class="tool-res"><p style="color:var(--green);font-weight:600;margin-bottom:8px"><i class="fas fa-check-circle"></i> File protected!</p><div id="binDetails" style="margin-bottom:12px"></div><a class="tool-btn" id="binDl" download><i class="fas fa-download"></i> Download Protected File</a></div>');
    if (!C) return;
    var fI = C.querySelector('#binFile'), zone = C.querySelector('#binZone'), btn = C.querySelector('#binApply'),
        res = C.querySelector('#binRes'), dl = C.querySelector('#binDl'), fileData = null, fileName = '';
    zone.addEventListener('click', function() { fI.click(); });
    fI.addEventListener('change', function(e) {
      if (!e.target.files.length) return;
      var file = e.target.files[0]; fileName = file.name;
      var r = new FileReader(); r.onload = function(ev) { fileData = new Uint8Array(ev.target.result); btn.style.display = 'inline-flex'; };
      r.readAsArrayBuffer(file);
    });
    function crc32(buf) {
      var t = crc32.t || (crc32.t = (function() { var t = []; for (var n = 0; n < 256; n++) { var c = n; for (var k = 0; k < 8; k++) c = (c & 1) ? (0xEDB88320 ^ (c >>> 1)) : (c >>> 1); t[n] = c; } return t; })());
      var c = 0xFFFFFFFF;
      for (var i = 0; i < buf.length; i++) c = (c >>> 8) ^ t[(c ^ buf[i]) & 0xFF];
      return (c ^ 0xFFFFFFFF) >>> 0;
    }
    function makeChunk(keyword, text) {
      var kw = new TextEncoder().encode(keyword), tx = new TextEncoder().encode(text), dl = kw.length + 1 + tx.length;
      var chunk = new Uint8Array(12 + dl), v = new DataView(chunk.buffer);
    ...
    btn.addEventListener('click', function() {
      if (!fileData) return;
      var level = C.querySelector('#binLevel').value, bytes = new Uint8Array(fileData), markers = [];
      if (fileName.endsWith('.png')) {
        var t1 = makeChunk('ai-training-opt-out', 'DO NOT USE FOR AI TRAINING'), t2 = makeChunk('noai', 'true');
        var nb = new Uint8Array(bytes.length + t1.length + t2.length);
        var ihdrLen = (bytes[4] << 24) | (bytes[5] << 16) | (bytes[6] << 8) | bytes[7], at = 12 + ihdrLen + 4;
        ...
        if (level === 'hard') {
          var t3 = makeChunk('poison', 'AI_SCRAPER_REJECT'), fb = new Uint8Array(bytes.length + t3.length);
        ...
        var header = '/* [NO-AI-TRAINING] Protected content. */\n', orig = new TextDecoder().decode(bytes);
        if (level !== 'soft') {
          var p = ''; for (var i = 0; i < orig.length; i++) { p += orig[i]; if (i % 3 === 0) p += '\u200B'; if (i % 7 === 0) p += '\u200D'; }
          bytes = new TextEncoder().encode(header + p);
          markers.push('Zero-width injection');
        } else { bytes = new TextEncoder().encode(header + orig); }
        markers.push('Header: NO-AI-TRAINING');
      }
      var blob = new Blob([bytes], { type: 'application/octet-stream' }), url = URL.createObjectURL(blob);
  ...
  AIO.skillDecay = function(el) {
    ...
    var C = initTool(el, '<div class="tool-sec"><label>Select Your Role</label><select class="tool-inp" id="sdRole">' + Object.keys(roles).map(function(r) { return '<option value="' + r + '">' + r + '</option>'; }).join('') + '</select></div><div class="tool-sec"><label>Last Major Skill Update</label><input type="date" class="tool-inp" id="sdDate" style="max-width:250px"/></div><button class="tool-btn" id="sdBtn"><i class="fas fa-heart-pulse"></i> Calculate Decay</button><div id="sdRes" class="tool-res"></div>');
    if (!C) return;
    C.querySelector('#sdBtn').addEventListener('click', function() {
      ...
      var remaining = Math.pow(0.5, ye / hl) * 100, decay = new Date(last.getTime() + hl * 365.25 * 864e5), critical = new Date(last.getTime() + hl * 2 * 365.25 * 864e5);
      var color = remaining > 70 ? 'var(--green)' : remaining > 40 ? 'var(--amber)' : 'var(--red)', status = remaining > 70 ? 'Current' : remaining > 40 ? 'Decaying' : 'Critical';
      var h = '<div style="text-align:center;margin-bottom:24px"><div style="font-size:4rem;font-weight:800;color:' + color + '">' + Math.round(remaining) + '%</div><p style="color:var(--text-muted)">Skill Relevance Remaining</p><span class="badge ' + (remaining > 70 ? 'badge-g' : remaining > 40 ? 'badge-y' : 'badge-r') + '">' + status + '</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">';
      [['HALF-LIFE', hl + ' years'],['50% DECAY', decay.toLocaleDateString()],['CRITICAL (25%)', critical.toLocaleDateString()],['YEARS ELAPSED', ye.toFixed(1)]].forEach(function(d) { h += '<div style="padding:16px;background:var(--bg-tertiary);border-radius:var(--radius-sm);text-align:center"><p style="font-size:.75rem;color:var(--text-muted);margin-bottom:4px">' + d[0] + '</p><p style="font-weight:700;font-size:1.1rem">' + d[1] + '</p></div>'; });
  ...
  AIO.hitlRoi = function(el) {
    var C = initTool(el, '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" class="hitlGrid"><style>@media(max-width:700px){.hitlGrid{grid-template-columns:1fr!important}}</style><div class="tool-sec"><label>Monthly Automation Savings ($)</label><input type="number" class="tool-inp" id="hitlSave" value="5000"/></div><div class="tool-sec"><label>Customers Affected / Month</label><input type="number" class="tool-inp" id="hitlCust" value="1000"/></div><div class="tool-sec"><label>Automation Error Rate (%)</label><input type="number" class="tool-inp" id="hitlErr" value="8" step="0.5"/></div><div class="tool-sec"><label>Avg Customer Lifetime Value ($)</label><input type="number" class="tool-inp" id="hitlCLV" value="500"/></div><div class="tool-sec"><label>Cost to Resolve Error ($)</label><input type="number" class="tool-inp" id="hitlResolve" value="25"/></div><div class="tool-sec"><label>Human Agent Cost / Hour ($)</label><input type="number" class="tool-inp" id="hitlAgent" value="35"/></div></div><button class="tool-btn" id="hitlBtn"><i class="fas fa-calculator"></i> Calculate True ROI</button><div id="hitlRes" class="tool-res"></div>');
    if (!C) return;
    C.querySelector('#hitlBtn').addEventListener('click', function() {
      var save = parseFloat(C.querySelector('#hitlSave').value) || 0, cust = parseFloat(C.querySelector('#hitlCust').value) || 0,
      ...
      [['Savings','$' + save.toLocaleString()],['Frustration Tax','$' + frustration.toFixed(0)],['Resolution Cost','$' + resCost.toFixed(0)],['True Net ROI','$' + netROI.toFixed(0)],['Hybrid ROI','$' + hybridROI.toFixed(0)]].forEach(function(d) { h += '<div style="text-align:center;padding:14px;background:var(--bg-tertiary);border-radius:var(--radius-sm)"><p style="font-size:.75rem;color:var(--text-muted)">' + d[0] + '</p><p style="font-weight:700;font-size:1.2rem">' + d[1] + '</p></div>'; });
  ...
  AIO.trustScore = function(el) {
    var C = initTool(el, '<div class="tool-sec"><label>Project Description</label><textarea class="tool-inp" id="tsDesc" placeholder="Describe the work you completed…" rows="3"></textarea></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" class="tsGrid"><style>@media(max-width:700px){.tsGrid{grid-template-columns:1fr!important}}</style><div class="tool-sec"><label>Hours Worked</label><input type="number" class="tool-inp" id="tsHours" value="40"/></div><div class="tool-sec"><label>Files Delivered</label><input type="number" class="tool-inp" id="tsFiles" value="5"/></div><div class="tool-sec"><label>Client Name</label><input class="tool-inp" id="tsClient" placeholder="Client or Company"/></div><div class="tool-sec"><label>Your Name</label><input class="tool-inp" id="tsName" placeholder="Your Name"/></div></div><button class="tool-btn" id="tsBtn"><i class="fas fa-certificate"></i> Generate Trust Certificate</button><div id="tsRes" class="tool-res"></div>');
    if (!C) return;
    C.querySelector('#tsBtn').addEventListener('click', async function() {
      var desc = C.querySelector('#tsDesc').value, hours = C.querySelector('#tsHours').value, files = C.querySelector('#tsFiles').value,
          client = C.querySelector('#tsClient').value, name = C.querySelector('#tsName').value;
      ...
      try {
        var enc = new TextEncoder(), kp = await crypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, ['sign', 'verify']);
  ...
  AIO.aiLiability = function(el) {
    var C = initTool(el, '<div class="tool-sec"><label>AI Model Temperature</label><input type="range" id="aiTemp" min="0" max="2" step="0.1" value="0.7" style="width:100%;accent-color:var(--accent)"/><p style="color:var(--text-muted);font-size:.85rem">Value: <span id="aiTv">0.7</span></p></div><div class="tool-sec"><label>Decision Domain</label><select class="tool-inp" id="aiDomain"><option value="content">Content Generation</option><option value="medical">Medical/Health</option><option value="financial">Financial</option><option value="legal">Legal Documents</option><option value="hiring">Hiring/HR</option><option value="safety">Safety-Critical</option></select></div><div class="tool-sec"><label>Human Review Rate (%)</label><input type="number" class="tool-inp" id="aiReview" value="50"/></div><div class="tool-sec"><label>Monthly AI Decisions</label><input type="number" class="tool-inp" id="aiDecisions" value="10000"/></div><div class="tool-sec"><label>Avg Claim Cost if Wrong ($)</label><input type="number" class="tool-inp" id="aiClaim" value="5000"/></div><button class="tool-btn" id="aiBtn"><i class="fas fa-gavel"></i> Calculate Risk</button><div id="aiRes" class="tool-res"></div>');
    if (!C) return;
    ...
    C.querySelector('#aiBtn').addEventListener('click', function() {
      var temp = parseFloat(C.querySelector('#aiTemp').value), domain = C.querySelector('#aiDomain').value,
      ...
      var unreviewed = dec * (1 - review), expectedErrors = unreviewed * hallRate, monthlyRisk = expectedErrors * claim, annualRisk = monthlyRisk * 12, insCost = annualRisk * 0.04;
      ...
      var h = '<div style="text-align:center;margin-bottom:20px"><span style="font-size:3rem;font-weight:800;color:' + color + '">$' + (annualRisk / 1000).toFixed(0) + 'K</span><p style="color:var(--text-muted);font-size:.85rem">Annual Liability Exposure</p><span class="badge ' + (riskLevel === 'LOW' ? 'badge-g' : riskLevel === 'MEDIUM' ? 'badge-y' : 'badge-r') + '">' + riskLevel + ' RISK</span></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:20px">';
      [['Hallucination', (hallRate * 100).toFixed(1) + '%'], ['Unreviewed', (unreviewed).toLocaleString() + '/mo'], ['Expected Errors', expectedErrors.toFixed(0) + '/mo'], ['Monthly Risk', '$' + monthlyRisk.toFixed(0)], ['Est. Insurance', '$' + insCost.toFixed(0) + '/yr']].forEach(function(d) { h += '<div style="text-align:center;padding:14px;background:var(--bg-tertiary);border-radius:var(--radius-sm)"><p style="font-size:.75rem;color:var(--text-muted)">' + d[0] + '</p><p style="font-weight:700">' + d[1] + '</p></div>'; });
      h += '</div><div style="padding:16px;background:var(--bg-tertiary);border-radius:var(--radius-sm);border-left:3px solid ' + color + '"><p style="font-size:.85rem;color:var(--text-secondary)"><strong>Mitigation:</strong> ';
  ...
  AIO.remoteTaxSync = function(el) {
    ...
    C.querySelector('#taxBtn').addEventListener('click', () => {
      ...
      h '<h3 style="margin-bottom:16px"><i class="fas fa-calendar-days" style="color:var(--accent)"></i> Year Summary</h3>';
  ...
  AIO.empathyMatrix = function(el) {
    ...
    function renderQ() {
      ...
      h `<div class="tool-box" style="margin-bottom:16px"><p style="font-size:var(--text-xs);color:var(--text-muted);margin-bottom:8px">Scenario ${current+1} of ${scenarios.length}</p><h3 style="margin-bottom:20px;line-height:1.5">${s.q}</h3>`;
    ...
    function showResults() {
      ...
      h `<div style="text-align:center;margin-bottom:24px"><div style="font-size:4rem;font-weight:800;color:${color}">${pct}%</div><p style="color:var(--text-muted)">Empathy Score</p><span class="badge ${pct>=60?'badge-g':pct>=40?'badge-y':'badge-r'}">${level}</span></div>`;
  ...
  AIO.vendorLockin = function(el) {
    ...
    C.querySelector('#vlBtn').addEventListener('click', () => {
      ...
      h `<div style="text-align:center;margin-bottom:20px"><div style="font-size:3.5rem;font-weight:800;color:${color}">${lockin}%</div><p style="color:var(--text-muted);font-size:.85rem">Vendor Lock-in Severity</p><span class="badge ${lockin<30?'badge-g':lockin<60?'badge-y':'badge-r'}">${level}</span></div>`;
  ...
  AIO.relatabilityScore = function(el) {
    ...
    C.querySelector('#rsBtn').addEventListener('click', () => {
      ...
      h `<div style="text-align:center;margin-bottom:24px"><div style="font-size:3.5rem;font-weight:800;color:${color}">${overall}%</div><p style="color:var(--text-muted);font-size:.85rem">Relatability Score</p><span class="badge ${overall>=70?'badge-g':overall>=40?'badge-y':'badge-r'}">${verdict}</span></div>`;
  ...
  AIO.burnoutSentiment = function(el) {
    ...
    C.querySelector('#bsBtn').addEventListener('click', () => {
      ...
      h '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:16px;margin-bottom:20px">';
  ...
  AIO.trustTax = function(el) {
    ...
    C.querySelector('#ttBtn').addEventListener('click', () => {
      ...
      h `<div style="text-align:center;margin-bottom:20px"><div style="font-size:3.5rem;font-weight:800;color:${color}">${trustScore}</div><p style="color:var(--text-muted);font-size:.85rem">Audience Trust Score</p></div>`;
      h += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:20px">';
  ...
  AIO.voiceHarmonizer = function(el) {
    ...
    C.querySelector('#vhBtn').addEventListener('click', () => {
      ...
      h '<h3 style="margin-bottom:16px">Harmonization Guide</h3>';
  ...
  AIO.visualContrastSchema = function(el) {
    ...
    function analyze() {
      ...
      h '<h3 style="margin-bottom:16px">Visual Analysis</h3><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">';
  ...
  AIO.microIntentTree = function(el) {
    ...
    C.querySelector('#miBtn').addEventListener('click', () => {
      ...
      h `<p style="margin-bottom:16px;color:var(--text-secondary);font-size:.85rem">Generated <strong>${intents.length}</strong> micro-intents from "<strong>${seed}</strong>"</p>`;
  ...
  AIO.byteToCarbon = function(el) {
    ...
    C.querySelector('#bcBtn').addEventListener('click', () => {
      ...
      h `<div style="text-align:center;margin-bottom:20px"><div style="font-size:3rem;font-weight:800;color:${color}">${grade}</div><p style="color:var(--text-muted);font-size:.85rem">Carbon Grade</p></div>`;
  ...
  AIO.honeypotVerify = function(el) {
    ...
    C.querySelector('#hpBtn').addEventListener('click', () => {
      ...
      h '<div class="tool-sec"><label>Preview</label><div style="padding:24px;background:#1a1a2e;border-radius:var(--radius-md);color:#fff;margin-bottom:16px"><h3 style="margin-bottom:12px">Human Verification</h3>';
      shuffled.forEach(p => {
  ...
  AIO.ethicalBiasSim = function(el) {
    ...
    C.querySelector('#ebBtn').addEventListener('click', () => {
      ...
      h `<div style="text-align:center;margin-bottom:20px"><div style="font-size:3.5rem;font-weight:800;color:${color}">${grade}</div><p style="color:var(--text-muted);font-size:.85rem">Bias Audit Score: ${score}/100</p><p style="font-size:.85rem;color:var(--text-secondary)">${issues} of ${totalTests} scenarios flagged</p></div>`;
  ...
  AIO.burstinessStylometer = function(el) {
    ...
    C.querySelector('#bsBtn').addEventListener('click', () => {
      ...
      h `<div style="text-align:center;margin-bottom:20px"><div style="font-size:3.5rem;font-weight:800;color:${color}">${humanScore}%</div><p style="color:var(--text-muted);font-size:.85rem">Human Writing Probability</p><span class="badge ${humanScore>60?'badge-g':humanScore>35?'badge-y':'badge-r'}">${verdict}</span></div>`;
      h += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;margin-bottom:20px">';
  ...
  AIO.sgeGapFinder = function(el) {
    ...
    C.querySelector('#sgBtn').addEventListener('click', () => {
      ...
      h `<h3 style="margin-bottom:16px">Information Gaps for: "<span style="color:var(--accent)">${topic}</span>"</h3>`;
